import React from 'react';
import "./homeHeroImages.css";
import assets from '../../assets/assets';

const HomeHeroImages = () => {
  return (
    <div className='homeHeroImages'>
        <div className="homeHeroImagesContainer">

            {/* Div */}
            <div className="homeHeroImagesContainerDiv">
                <div className="homeHeroImagesContainerDivContainer">
                    <div className="homeHeroImagesContainerDivContainerDiv" />
                </div>
            </div>




            {/* Top Part */}
            <div className="homeHeroImagesContainerTop">
                <div className="homeHeroImagesContainerTopContainer">





                    {/* Element */}
                    <div className="homeHeroImagesContainerTopContainerElement">
                        <div className="homeHeroImagesContainerTopContainerElementContainer">
                            <div className="homeHeroImagesContainerTopContainerElementContainerDiv">
                                <img src={assets.HomeHeroImagesOne} alt="" className='homeHeroImagesContainerTopContainerElementContainerDivImage' />
                            </div>
                        </div>
                    </div>

                    <div className="homeHeroImagesContainerTopContainerDiv">
                        <div className="homeHeroImagesContainerTopContainerDivContainer">
                            <div className="homeHeroImagesContainerTopContainerDivContainerDiv" />
                        </div>
                    </div>



                    {/* Element */}
                    <div className="homeHeroImagesContainerTopContainerElement">
                        <div className="homeHeroImagesContainerTopContainerElementContainer">
                            <div className="homeHeroImagesContainerTopContainerElementContainerDiv">
                                <img src={assets.HomeHeroImagesTwo} alt="" className='homeHeroImagesContainerTopContainerElementContainerDivImage' />
                            </div>
                        </div>
                    </div>

                    <div className="homeHeroImagesContainerTopContainerDiv">
                        <div className="homeHeroImagesContainerTopContainerDivContainer">
                            <div className="homeHeroImagesContainerTopContainerDivContainerDiv" />
                        </div>
                    </div>



                    {/* Element */}
                    <div className="homeHeroImagesContainerTopContainerElement">
                        <div className="homeHeroImagesContainerTopContainerElementContainer">
                            <div className="homeHeroImagesContainerTopContainerElementContainerDiv">
                                <img src={assets.HomeHeroImagesThree} alt="" className='homeHeroImagesContainerTopContainerElementContainerDivImage' />
                            </div>
                        </div>
                    </div>

                    <div className="homeHeroImagesContainerTopContainerDiv">
                        <div className="homeHeroImagesContainerTopContainerDivContainer">
                            <div className="homeHeroImagesContainerTopContainerDivContainerDiv" />
                        </div>
                    </div>




                    {/* Element */}
                    <div className="homeHeroImagesContainerTopContainerElement">
                        <div className="homeHeroImagesContainerTopContainerElementContainer">
                            <div className="homeHeroImagesContainerTopContainerElementContainerDiv">
                                <img src={assets.HomeHeroImagesFour} alt="" className='homeHeroImagesContainerTopContainerElementContainerDivImage' />
                            </div>
                        </div>
                    </div>





                </div>
            </div>




            {/* Div */}
            <div className="homeHeroImagesContainerDiv">
                <div className="homeHeroImagesContainerDivContainer">
                    <div className="homeHeroImagesContainerDivContainerDiv" />
                </div>
            </div>

            {/* Bottom Part */}
            <div className="homeHeroImagesContainerBottom">
                <div className="homeHeroImagesContainerBottomContainer">

                    {/* Element */}
                    <div className="homeHeroImagesContainerTopContainerElement">
                        <div className="homeHeroImagesContainerTopContainerElementContainer">
                            <div className="homeHeroImagesContainerTopContainerElementContainerDiv">
                                <img src={assets.HomeHeroImagesFive} alt="" className='homeHeroImagesContainerTopContainerElementContainerDivImage' />
                            </div>
                        </div>
                    </div>

                    <div className="homeHeroImagesContainerTopContainerDiv">
                        <div className="homeHeroImagesContainerTopContainerDivContainer">
                            <div className="homeHeroImagesContainerTopContainerDivContainerDiv" />
                        </div>
                    </div>
                    

                    {/* Element */}
                    <div className="homeHeroImagesContainerTopContainerElement">
                        <div className="homeHeroImagesContainerTopContainerElementContainer">
                            <div className="homeHeroImagesContainerTopContainerElementContainerDiv">
                                <img src={assets.HomeHeroImagesSix} alt="" className='homeHeroImagesContainerTopContainerElementContainerDivImage' />
                            </div>
                        </div>
                    </div>

                    <div className="homeHeroImagesContainerTopContainerDiv">
                        <div className="homeHeroImagesContainerTopContainerDivContainer">
                            <div className="homeHeroImagesContainerTopContainerDivContainerDiv" />
                        </div>
                    </div>


                    {/* Element */}
                    <div className="homeHeroImagesContainerTopContainerElement">
                        <div className="homeHeroImagesContainerTopContainerElementContainer">
                            <div className="homeHeroImagesContainerTopContainerElementContainerDiv">
                                <img src={assets.HomeHeroImagesEight} alt="" className='homeHeroImagesContainerTopContainerElementContainerDivImage' />
                            </div>
                        </div>
                    </div>

                    <div className="homeHeroImagesContainerTopContainerDiv">
                        <div className="homeHeroImagesContainerTopContainerDivContainer">
                            <div className="homeHeroImagesContainerTopContainerDivContainerDiv" />
                        </div>
                    </div>


                    {/* Element */}
                    <div className="homeHeroImagesContainerTopContainerElement">
                        <div className="homeHeroImagesContainerTopContainerElementContainer">
                            <div className="homeHeroImagesContainerTopContainerElementContainerDiv">
                                <img src={assets.HomeHeroImagesSeven} alt="" className='homeHeroImagesContainerTopContainerElementContainerDivImage' />
                            </div>
                        </div>
                    </div>

                </div>
            </div>

            {/* Div */}
            <div className="homeHeroImagesContainerDiv">
                <div className="homeHeroImagesContainerDivContainer">
                    <div className="homeHeroImagesContainerDivContainerDiv" />
                </div>
            </div>

        </div>
    </div>
  )
}

export default HomeHeroImages